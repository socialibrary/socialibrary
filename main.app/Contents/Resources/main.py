...
import locale
import urllib2
from elementtree.ElementTree import parse,Element,SubElement,tostring
from AWSQuerySigner import AWSQS

# fill in your AWS Access Key ID (AWSId) and AWS Secret Key (AWSKey)
AWSId = "AKIAJKHHXP722DJWPVGQ"
AWSKey = "oZcg6o9oDyXbp1kITjjR2q9V3wnYw/gsiPr7M9Sm"
...
class BookHandler(BaseHandler):
    """Displays the addition screen for books"""
    @user_required
    def get(self):
        if self.user:
            self.render(u'searchbook')
        else:
            self.render(u'welcome')
...
class BookSearchHandler(BaseHandler):
    ...
    def post(self):
        if self.user:
            keyword = self.request.POST[u'booktitle'].strip()
            if keyword =="":
                keyword = self.request.POST[u'author'].strip()
            if keyword =="":
                self.render(u'searchbook',nokeyword='true')
            ...
            for i in range(1,5):
                ...
                awsqs = AWSQS( 'GET', endpoint, uri, query, AWSKey )
                
                url = awsqs.signedrequest
                '''
                try:
                    doc=parse(urlfetch.fetch(url, method=urlfetch.GET, deadline=10))
                ...
                item = doc.getroot()
                ns = "{http://webservices.amazon.com/AWSECommerceService/2005-10-05}"
                g= item.find("%sItems" % ns)

                #for k in item[1][4]:
                #	print k

                for e in range(3,len(g)):
                    #ProductType
                    pg = g[e].find(ns+"ItemAttributes").find(ns+"ProductGroup").text
                    if pg == "Book":
                        #ASIN
                        #print g[e].find(ns+"ASIN").text,
                        #Author
                        try:
                            author = g[e].find(ns+"ItemAttributes").find(ns+"Author").text
                        except AttributeError:
                            author = "No author"
                        #Title
                        #print g[e].find(ns+"ItemAttributes").find(ns+"Title").text,
                        #largeimageurl
                        #print g[e].find(ns+"LargeImage").find(ns+"URL").text,
                        #detailpageurl
                        #print g[e].find(ns+"DetailPageURL").text
                        logging.info('author is')
                        logging.info(author)
                        book = Book( 
                                    asin = g[e].find(ns+"ASIN").text,
                                    title = g[e].find(ns+"ItemAttributes").find(ns+"Title").text,
                                    author = author,
                                    image = g[e].find(ns+"LargeImage").find(ns+"URL").text,
                                    pageurl = g[e].find(ns+"DetailPageURL").text)
                        Books.append(book)
            
            self.render(u'searchbook',books=Books)
...
class GameHandler(BaseHandler):
    """Displays the addition screen for games"""
    def get(self):
        if self.user:
            self.render(u'searchbook')
        else:
...
class MovieHandler(BaseHandler):
    """Displays the addition screen for movies"""
    def get(self):
        if self.user:
            self.render(u'searchbook')
        else:
